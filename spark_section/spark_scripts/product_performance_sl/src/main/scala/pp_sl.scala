...
object pp_sl {
    ...
    def main(args: Array[String]) = {
        ...
        val spark = SparkSession.builder()
                        .appName("ETL-data_lake_to_product_performance_serving_layer")
        ...
        val sl_product_performance = products_df.join(order_details_df, 
                                                ...
                                                .groupBy("p_row_id", "product_name", "category", "sub_category", "year", "quarter", "month", "day")
                                                    .agg(avg("product_price").as("product_price"), 
                                                         ...
                                                         avg("tweet_sentiment_score").as("sentiment_score"), 
                                                         avg("review_sentiment_score").as("review_perception"))
                                                .withColumnRenamed("p_row_id", "row_id")

                                    
        println("CASSANDRA - Loading Data to Cassandra")
        sl_product_performance
